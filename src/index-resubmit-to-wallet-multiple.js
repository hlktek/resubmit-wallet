...
async function resubmit(ticketId) {
    const ticketInfos = await getTicketInfo(ticketId);
    if (ticketInfos && ticketInfos.length > 0) {
        for (let i = 0; i < ticketInfos.length; i++) {
            const requests = ticketInfos[i];
            const request = requests[0];
            const callGamemsResult = await callWallet(requests);
            resubmitInfo.push({
                ticketId,
                userId : request.uid,
        ...
        if (responsse.status === 200) {
            ...
            if (sources && sources.length > 0) {
                return sources.map( source => {
                    return source._source.logInfo.request;
                })
            }
        }
    } catch (err) {
